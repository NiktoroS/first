...
#            exit("invalid redirect url");
    ...
    static protected function crop($dir)
    {
        foreach (scandir($dir) as $file) {
            $output = [];
            if (!preg_match('/^(\d+)\.png$/', $file, $output)) {
                continue;
            }
            $imageSize = getimagesize($dir . DS . $file);

            $imageIn = imagecreatefrompng($dir . DS . $file);
            ...
            $imageOut = imagecreatetruecolor($xMax + 1 - $xMin, $yMax + 1 - $yMin);
            $white    = imagecolorallocate($imageOut, 255, 255, 255);
            for ($x = $xMin; $x <= $xMax; $x ++) {
                for ($y = $yMin; $y <= $yMax; $y ++) {
                    $rgb = imagecolorat($imageIn, $x, $y);
                    if ($rgb) {
                        imagesetpixel($imageOut, $x - $xMin, $y - $yMin, $white);
                        continue;
                    }
                }
            }
            imagepng($imageOut, $dir . DS . "crop" . DS . $file);
        }
    }

    static protected function grabFromImage($tmpFile, $xFrom, $yFrom, $xTo, $yTo)
    {
        $imgIn  = imagecreatefrompng($tmpFile);
        $imgOut = imagecreatetruecolor($xTo - $xFrom, $yTo - $yFrom);
        ...
        for ($x = $xFrom; $x < $xTo; $x ++) {
            for ($y = $yFrom; $y < $yTo; $y ++) {
                ...
                $b = $rgb & 0xFF;
                if ($r < 1 && $g < 1 && $b < 1) {
                    imagesetpixel($imgOut, $x - $xFrom, $y - $yFrom, $black);
                } else {
                    imagesetpixel($imgOut, $x - $xFrom, $y - $yFrom, $white);
                }
            }
        }
        imagepng($imgOut, TMP_DIR . time() . ".png");
    }
