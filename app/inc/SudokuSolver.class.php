...
class SudokuSolver
{
    ...
    static function findPossibleValues($rowIndex, $columnIndex, $puzzle)
    {
        ...
        $values = array_diff($values, self::getRowValues($rowIndex, $puzzle));
        $values = array_diff($values, self::getColumnValues($columnIndex, $puzzle));
        $values = array_diff($values, self::getBlockValues($rowIndex, $columnIndex, $puzzle));
