...
/**/
    ...
    $cntParametr = count($row["Parameters"]["Next"]["Parameter"]);
    if (!$cntParametr) {
        var_dump($row);
        exit;
    }
    $cntChoise = count($row["Parameters"]["Next"]["Parameter"][$cntParametr - 1]["Predefined"]["Choice"]);
    if ($cntChoise < 10) {
        var_dump($row);
        exit;
    }
    ...
    $step ++;
    $params[$step] = [
        13 => $row["Parameters"]["Next"]["Parameter"][$cntParametr - 1]["Predefined"]["Choice"][5]["@attributes"]["value"],
//        13 => $row["Parameters"]["Next"]["Parameter"][$cntParametr - 1]["Predefined"]["Choice"][0]["@attributes"]["value"],
    ];
...
//    var_dump(resultToRow($result));
    ...
    $getCheck   = "/?Amount=26500&" . paramStr($paramsCheck) . "&PaymExtId={$extId}&PaymSubjTp={$PaymSubjTp}&RequestId={$requestId}&TermID={$termId}&TermType=006-22&function=Check&usr_mail={$usr_mail}&user_ip={$user_ip}&protocol_type=pip";
    $result  = $browser->request("{$uri}{$getCheck}", "", $headerRows);
    sleep(1);

    $getPatment = "/?Amount=26500&" . paramStr($paramsCheck) . "&PaymExtId={$extId}&PaymSubjTp={$PaymSubjTp}&RequestId={$requestId}&TermID={$termId}&TermType=006-22&function=Payment&usr_mail={$usr_mail}&user_ip={$user_ip}&protocol_type=pip";
