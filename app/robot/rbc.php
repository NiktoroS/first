...
$quoteRows = [
    "USD"    => ["selt", 338247, "delay", "1"], // 59111
    "EUR"    => ["selt", 338243, "delay", "1"], // 59090
...
try {
    ...
    foreach ($quoteRows as $var => $vals) {
        $url = "https://www.rbc.ru/quote/ticker/" . $vals[1];
        $browser->request($url);
        foreach(array ("d", "w", "m", "y") as $band) {
            $url   = "https://www.rbc.ru/quote/data/ticker/graph/" . $vals[1] . "/" . $band . "/?_=" . intval(microtime(true) * 1000);
...
#                var_dump($array);
            ...
            foreach ($array["result"]["data"] as $row) {
                if (empty($rbcRows[$row[3]])) {
                    $rbcRows[$row[3]] = [
                        "created" => date("Y-m-d H:i:s", $row[3] / 1000)
                    ];
                }
                if ($row[4]) {
